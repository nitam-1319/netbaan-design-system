# Working with @netbaan-project/ui (rules for AI agents)

`@netbaan-project/ui` is a **sealed** React component library (the AEGIS design system).
...
1. **Discover before you build.** Before creating ANY UI element, consult the
   ...
   shipped at the package root (`node_modules/@netbaan-project/ui/`). If a component
...
3. **The implementation is immutable.** Never edit anything under
   `node_modules/@netbaan-project/ui`. It is compiled, minified, and has no source maps
...
import "@netbaan-project/ui/styles.css";

// Components — tree-shakeable named imports:
import { Button, Dialog, ThemeProvider } from "@netbaan-project/ui";
```
...
- **Fonts:** optional. `import "@netbaan-project/ui/fonts.css"` if you want the bundled
