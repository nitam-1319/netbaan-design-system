...
- 🟡 **Phase 8 — Release** — SET UP + migrated to company org (publish pending). Package **`@netbaan-project/ui`** (scope = the `netbaan-project` GitHub org, as GitHub Packages requires). Target: **GitHub Packages (private)** — access follows the repo, nothing on public npm. `publishConfig.registry` pinned to `npm.pkg.github.com` + committed `.npmrc` (`@netbaan-project` scope, token via `NODE_AUTH_TOKEN` env — no secret committed). `prepack` runs the full build on publish; `files` allowlist keeps source out. `RELEASING.md` documents the flow. **`npm publish --dry-run` confirms** `@netbaan-project/ui` → "Publishing to https://npm.pkg.github.com … restricted access".
  - **Migration:** repo moved from personal `nitam-1319/netbaan-design-system` to company **`netbaan-project/front-npm`**. Full history pushed to `origin/main` (empty clean repo); personal repo kept as the `personal` git remote (backup). All name references regenerated/updated to `@netbaan-project/ui`.
  - **Published ✅** — `@netbaan-project/ui@0.0.2` is live on the org's private GitHub Packages, released by `github-actions[bot]` via `.github/workflows/release.yml` (repo `GITHUB_TOKEN`, `packages: write`). Personal-account `npm publish` hit `403 create_package` (org-membership limitation, not token scope — the PAT had `repo, write:packages`); publishing from CI as the repository resolved it. Trigger = push a `vX.Y.Z` tag (or Actions → Run workflow).

**Project complete: Phases 1–8 done. `@netbaan-project/ui` is a sealed, immutable, tree-shakeable, SSR-safe React library published privately for the company, shipping its AI knowledge layer.**
...
Phases 1–7 complete. Phase 8 set up and migrated to the company org (`@netbaan-project/ui` on `netbaan-project/front-npm`, pushed to `origin/main`); only the outward-facing `npm publish` remains for the user to run.
