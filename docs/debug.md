...
## ⏳ D4 · P1 — Fonts are a hard Google Fonts dependency

`fonts.css` is one `@import url(fonts.googleapis.com…)` for four families / 15
...
Self-host, subset, ship woff2 variable fonts in `dist/fonts/`.

> **Correction.** The original rewrote `fonts.css` only. **`src/index.css:3` has
> the identical Google Fonts `@import`**, so the dev app and Storybook would
> have kept the third-party request. Both files must change.
