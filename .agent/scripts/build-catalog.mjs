...
for (const file of files) {
  ...
  entries.push({
    ...
    exports: values,
    typeExports: types,
    import: `import { ${pascal(mod)} } from "${PKG}";`,
  });
}
...
let md = `# ${PKG} — Component Catalog\n\n`;
md += `AUTO-GENERATED by \`.agent/scripts/build-catalog.mjs\`. ${entries.length} components.\n\n`;
md += `> For AI agents: **consult this catalog before building any UI.** If a component here fits, use it — do not recreate it. Compose from existing components. Import via \`import { X } from "${PKG}"\`. Components are sealed: no \`className\`, configure through typed props only.\n\n`;
md += `## Index\n\n`;
